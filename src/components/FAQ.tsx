...
// `text` is the plain-text answer used for the FAQ rich-results schema; `a` is
// the (optionally richer) version rendered on the page.
const faqs: { q: string; text: string; a: React.ReactNode }[] = [
  ...
  },
  {
    ...
    text: 'Yes. I offer a free 15-minute consultation so we can connect and make sure we’re a good fit before you commit to anything.',
  ...
  },
  {
    ...
    text: 'Individual sessions are about 50 minutes. An initial diagnostic evaluation runs about an hour.',
  ...
  },
  {
    ...
    text: 'Adolescents, young adults, and adults navigating anxiety, depression, trauma, and life’s transitions.',
    a: 'Adolescents, young adults, and adults navigating anxiety, depression, trauma, and life’s transitions.',
  },
  {
    q: 'How do I get started?',
    text: 'Click “Request an Appointment” to book your free consultation or first session through the secure portal — it only takes a few minutes.',
            ...
            {faqs.map((f) => (
              <details key={f.q} className="group border-b border-[var(--theme-border-subtle)] last:border-b-0">
                <summary className="flex cursor-pointer list-none items-center justify-between gap-4 px-6 py-5 font-display text-lg font-semibold text-[var(--theme-text-primary)] transition hover:text-[var(--brand-teal)] [&::-webkit-details-marker]:hidden">
                  {f.q}
                  <ChevronDown className="h-5 w-5 flex-none text-[var(--brand-gold-ink)] transition group-open:rotate-180" />
                </summary>
